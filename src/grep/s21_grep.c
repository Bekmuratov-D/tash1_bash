...
void parser(int argc, char *argv[], flags *A) {
  ...
  if (A->e || A->f) {
    A->str_regexec[strlen(A->str_regexec) - 1] = '\0';  // Удаление символа "|"
  }
  ...
  if (file != NULL) {
    while (fgets(A->str, BUFFER, file) != NULL) {
      ...
      if ((!match || A->v) && A->c_flag && !A->l && !A->h && !A->c && !A->f)
        printf("%s:", argv[optind]);  // вывод имени файла
      if (!match) counter++;          // счетчик совпадений
      if (A->v) match = !match;  // инвертирование совпадений
