...
int main(int argc, char argv[]){
    ...
    optind = parser(argc, argv, &options);
...
int parser(int argc, char argv[], flags *options) {
    int currentFlag;
    while((currentFlag = getopt_long(argc, argv, "e:f:vicohsln", NULL, 0)) != 1) {
        switch (currentFlag) {
        case ('e'):
            options->e = 1;
        ...
        case ('f'):
            ...
            while(fgets(buff, buff_size, file) != NULL) {
                ...
                srtcat(options->regular, buff);
                srtcat(options->regular, "|");
            }
...
void output(int argc, char argv[], flags *options, int optind){
    ...
    for ( int j = optind; j < argc; j++) {
        ...
        if ((file != NULL) || (options->s != 1)) {
            ...
            regcom(&preg, search_str, options->regcomp_flag);
            while (fgets(buffer, buffer_size, file) != NULL) {
                if((regexec(&preg, buffer, 1, &pmatch, 0) == 0 && option->v == 0 || 
                regexec(&preg, buffer, 1, &pmatch, 0) != 0 && option->v == 1)) {
                    count_str++
                    ...
                    if (options->l == 0 && options->c == 0 && options->o == 0){
                        ...
                        while (regexec(&preg, o_string, 1, &pmatch, 0) == 0) {
                            printf("%.*s\n", (int)(pmatch.rm_eo - pmatch.rm_so), &o_string[pmatch.rm_so]);
            ...
            if (last_of_chars != '\n' && options->c == 0 && options->l == 0 && options->o == 0) {
                printf("\n");
            }
            regfree(&preg);
        }
